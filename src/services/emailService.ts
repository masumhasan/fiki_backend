...
/**
 * Send Account Registration OTP Email via Hostinger SMTP
 */
export async function sendRegistrationOtpEmail(
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Verify Your Fiki Transit Account</title>
      <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f7fb; margin: 0; padding: 20px; color: #172033; }
        .card { max-width: 520px; margin: 30px auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(8, 37, 82, 0.08); border: 1px solid #e1e6ee; }
        .header { background: #082552; padding: 28px 24px; text-align: center; color: #ffffff; }
        .header h1 { margin: 0; font-size: 22px; font-weight: 700; letter-spacing: 0.5px; }
        .header p { margin: 6px 0 0; font-size: 13px; color: #e4973c; font-weight: 600; letter-spacing: 1px; }
        .body { padding: 32px 28px; text-align: center; }
        .greeting { font-size: 16px; font-weight: 600; margin-bottom: 12px; color: #172033; text-align: left; }
        .text { font-size: 14px; line-height: 1.6; color: #52647e; margin-bottom: 24px; text-align: left; }
        .otp-box { background: #f0f4fa; border: 2px dashed #082552; border-radius: 12px; padding: 18px; margin: 24px 0; text-align: center; }
    ...
    <body>
      <div class="card">
        ...
        <div class="body">
          ...
          <div class="text">
            Thank you for registering with <strong>Fiki Transit</strong>. To verify your email address and complete your account creation, please enter the 6-digit verification code below:
          </div>
          ...
          <div class="text">
            If you did not initiate this registration, please disregard this email. Your email will not be registered without entering this code.
          </div>
        </div>
        <div class="footer">
          <p>© ${new Date().getFullYear()} Fiki Transit. All rights reserved.</p>
  ...
  try {
    const info = await transporter.sendMail({
      from: `"Fiki Transit Security" <${fromEmail}>`,
      to: toEmail,
      subject: `${otp} is your Fiki Transit verification code`,
      html: htmlContent,
    });

    console.log(`[EMAIL] Registration OTP email sent to ${toEmail}. Message ID: ${info.messageId}`);
    return true;
  } catch (error) {
    console.error("[EMAIL ERROR] Failed to send registration OTP email via Hostinger SMTP:", error, toEmail);
    return false;
  }
}
