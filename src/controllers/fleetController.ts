    ...
    try {
      ...
      const reports = shifts.map((s: any) => {
        ...
        return {
          ...
          startPhotoUrls: Array.isArray(s.startPhotoUrls) && s.startPhotoUrls.length > 0 ? s.startPhotoUrls : (s.startPhotoUrl ? [s.startPhotoUrl] : []),
          endPhotoUrls: Array.isArray(s.endPhotoUrls) && s.endPhotoUrls.length > 0 ? s.endPhotoUrls : (s.endPhotoUrl ? [s.endPhotoUrl] : []),
    ...
    try {
      ...
      res.status(200).json({
        ...
        data: {
          ...
          startPhotoUrls: Array.isArray(shift.startPhotoUrls) && shift.startPhotoUrls.length > 0 ? shift.startPhotoUrls : (shift.startPhotoUrl ? [shift.startPhotoUrl] : []),
          endPhotoUrls: Array.isArray(shift.endPhotoUrls) && shift.endPhotoUrls.length > 0 ? shift.endPhotoUrls : (shift.endPhotoUrl ? [shift.endPhotoUrl] : []),
